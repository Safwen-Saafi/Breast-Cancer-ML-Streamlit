...
def get_clean_data():
    # Adjust the path to point to the root-level data folder
    data_path = Path(__file__).resolve().parent.parent / "data/data.csv"
    data = pd.read_csv(data_path)
    data = data.drop(['Unnamed: 32', 'id', 'fractal_dimension_mean', 'texture_se', 'smoothness_se',
                      'compactness_se', 'concavity_se', 'symmetry_se', 'fractal_dimension_se',
                      'fractal_dimension_worst', 'symmetry_mean', 'smoothness_mean'], axis=1)
...
def add_predictions(input_data):
    # Adjust the paths to point to the root-level model folder
...
def main():
    ...
    # Adjust the path to point to the root-level style.css file
